...
static void button_press_up_cb(void *arg, void *data) {
    // 按键弹起
    if (press_rotation != 1) press_rotation = 0;
...
static void button_single_click_cb(void *arg, void *data) {
    ...
    if (press_rotation == 1) dial_publish(DIAL_STA_CLICK);
    press_rotation = 0;
...
static void button_long_press_start_cb(void *arg, void *data) {
    // 按键长按
    if (press_rotation == 1) dial_publish(DIAL_STA_LONG_PRESS);
...
static void foc_knob_inc_cb(void *arg, void *data) {
    ...
    if (press_rotation == 1) press_rotation = 2;
...
float get_motor_shaft_angle(void) { return motor.shaft_angle; }

float get_motor_shaft_velocity(void) { return motor.shaft_velocity; }
...
uint16_t get_fknob_shaft_angle(void) {
    ...
    adjusted_sub_position =
        state.angle_to_detent_center * state.position_width_radians;
...
void knob_event_init() {
    ...
    iot_button_register_cb(btn, BUTTON_DOUBLE_CLICK, button_double_click_cb,
                           NULL);
    iot_button_register_cb(btn, BUTTON_LONG_PRESS_START,
                           button_long_press_start_cb, NULL);
    iot_button_register_cb(btn, BUTTON_LONG_PRESS_UP, button_long_press_up_cb,
                           NULL);
}
