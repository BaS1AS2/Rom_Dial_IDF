...
void adc_read_task() {
    ...
    adc_continuous_handle_t handle = NULL;
    continuous_adc_init(adc_channel,
                        sizeof(adc_channel) / sizeof(adc_channel_t), &handle);
    ...
    while (1) {
         ...
         * event callback. This `ulTaskNotifyTake` will block when the data
         * processing in the task is fast. However in this example, the data
         * processing (print) is slow, so you barely block here.
         *
         * Without using this event callback (to notify this task), you can
         * still just call `adc_continuous_read()` here in a loop, with/without
         * a certain block timeout.
         */
        ...
        while (1) {
            ret = adc_continuous_read(handle, result, EXAMPLE_READ_LEN,
                                      &ret_num, 0);
            if (ret == ESP_OK) {
                // ESP_LOGI("TASK", "ret is %x, ret_num is %"PRIu32" bytes",
                // ret, ret_num); for (int i = 0; i < ret_num; i +=
                ...
                // ESP_LOGI(TAG, " Channel: %"PRIu32", Value: %"PRIu32,
                // chan_num, adc_val);
                /* Check the channel number validation, the data is invalid if
                 * the channel num exceed the maximum channel */
                // if (chan_num < SOC_ADC_CHANNEL_NUM(EXAMPLE_ADC_UNIT)) {
                // ESP_LOGI(TAG, "Unit: %s, Channel: %"PRIu32", Value: %"PRIu32,
                // unit, chan_num, adc_val); } else {
                //     ESP_LOGW(TAG, "Invalid data [%s_%"PRIu32"_%"PRIx32"]",
                //     unit, chan_num, adc_val);
                // }
                // }
                ...
                // which means there's no available data break;
            }
            ...
            // ESP_LOGI(TAG, "difftime
            // %f,now_angle:%f,last_angle:%f",difftime(now_time,last_time),now_angle,last_angle);
...
void power_on() { gpio_set_level(POWER_GPIO, 1); }
void power_off() {
    led_ring_stop();
...
const static int Battery_Level_Percent_Table[11] = {
    3400, 3650, 3700, 3740, 3760, 3795, 3840, 3910, 3980, 4070, 4200};
int toPercentage(uint32_t voltage) {
    if (voltage < Battery_Level_Percent_Table[0]) return 0;

    for (int i = 0; i < ARRAY_DIM(Battery_Level_Percent_Table); i++) {
        if (voltage < Battery_Level_Percent_Table[i])
            return i * 10 -
                   (10UL * (int)(Battery_Level_Percent_Table[i] - voltage)) /
                       (int)(Battery_Level_Percent_Table[i] -
                             Battery_Level_Percent_Table[i - 1]);
        ;
    }
    return 100;
}
