...
static const foc_knob_param_t screen1_foc_knob_param = {
    0, 0, 30 * PI / 180, 1, 1, 0.6, ""};
static void onFocus(lv_group_t* g) {
    ...
    for (uint8_t i = 0; i < cnt; i++) {
        ...
        if (i == mid_btn_index) {
            lv_obj_add_state(lv_obj_get_child(obt, 0),
...
static void Style_Init() {
    ...
    static const lv_style_prop_t style_prop[] = {LV_STYLE_WIDTH,
                                                 LV_STYLE_PROP_INV};

    static lv_style_transition_dsc_t trans;
    lv_style_transition_dsc_init(&trans, style_prop, lv_anim_path_overshoot,
                                 200, 0, NULL);
...
static void Item_Create(lv_obj_t* container, const char* name,
                        const void* img_src, const char* infos) {
    ...
    lv_obj_set_flex_align(icon, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER,
                          LV_FLEX_ALIGN_CENTER);
...
static void Create(lv_obj_t* root) {
    ...
    Item_Create(root, "USB HID", &ui_img_pc_png,
                "Surface Dial\n"
                "KeyBoard\n"
                "Mouse\n"
                "Media");
    Item_Create(root, "Setting", &ui_img_set_png,
                "Light\n"
                "Screen Lock Time\n"
                "Power Off Time\n"
                "Screen Direction\n"
    ...
    sprintf(system_info,
            "Name : Super Dial\nAuthor : 45coll\nChip Mac : %s\nUpdate : %s",
            sys_config.mac, sys_config.app_desc->date);
    Item_Create(root, "PowerOff", &ui_img_power_png, system_info);
    Item_Create(root, "Custom", &ui_img_customize_png, "Custom HID");
    Item_Create(root, "LedRing", &ui_img_light_png,
                "brightness\n"
                "RingSpeed\n");
...
void ui_Screen1_dial_event(uint8_t state) {
    switch (state) {
        case DIAL_STA_RELEASE:  // 0
            ESP_LOGI(TAG, "state: %d", state);
            break;
        case DIAL_STA_PRESS:  // 1
            ESP_LOGI(TAG, "state: %d", state);
            break;
        case DIAL_STA_R:  // 2
            ESP_LOGI(TAG, "state: %d", state);
        ...
        case DIAL_STA_L:  // 3
            ...
            enc_num--;
            break;
        case DIAL_STA_CLICK:            // 4
            int8_t index = icon_index;  // change screen1
            ESP_LOGI(TAG, "current:%d ,state: %d", index, state);
            switch (index) {
                case 0:  // usb hid
                    _ui_screen_change(&ui_Screen2, LV_SCR_LOAD_ANIM_FADE_ON,
                                      300, 0, &ui_Screen2_screen_init);
                    break;
                case 1:  // setting
                ...
                case 3:  // Customize HID Interface
                    _ui_screen_change(&ui_Screen3, LV_SCR_LOAD_ANIM_FADE_ON,
                                      300, 0, &ui_Screen3_screen_init);
                    break;
                case 4:  // led ring
                    _ui_screen_change(&ui_Led_Ring, LV_SCR_LOAD_ANIM_FADE_ON,
                                      300, 0, &ui_Screen_Led_Ring_screen_init);
                    break;
                default:
                    break;
            }
            break;
        case DIAL_STA_LONG_PRESS:  // 6
...
void ui_Screen1_screen_init(void) {
    ...
    lv_obj_add_event_cb(ui_Screen1, scr_Screen1_loaded_cb,
                        LV_EVENT_SCREEN_LOADED, &ui_Screen1);
    // lv_obj_add_event_cb(ui_Screen1, scr_Screen1_unloaded_cb,
    // LV_EVENT_SCREEN_UNLOADED, &ui_Screen1);
    lv_obj_set_style_bg_img_src(ui_Screen1, &ui_img_bg1_png,
                                LV_PART_MAIN | LV_STATE_DEFAULT);
