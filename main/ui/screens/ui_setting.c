...
static UI_SETTINGICON_INFO *ui_icon;
static struct {
...
static void ui_icon_info_init() {
    uint8_t id;
    ui_icon = heap_caps_malloc(ICON_CNT * sizeof(UI_SETTINGICON_INFO),
                               MALLOC_CAP_SPIRAM);
...
static void setl_label_info(uint8_t index) {
    ...
    switch (index) {
        ...
        case 1:  // 屏幕锁定时间
            ...
            lv_obj_add_flag(ui_label.wifi_roller, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(ui_label.bottom_label, LV_OBJ_FLAG_HIDDEN);
            lv_roller_set_options(ui_label.roller, SET_SCREEN_LOCK_TIME_SRC,
...
void ui_Screen_Setting_screen_init(void) {
    ...
    ui_icon_info_init();
    lv_obj_add_event_cb(ui_Screen_Setting, scr_Screen_Setting_loaded_cb,
                        LV_EVENT_SCREEN_LOADED, &ui_Screen_Setting);
    // lv_obj_add_event_cb(ui_Screen_Setting, scr_Screen_Setting_unloaded_cb,
    // LV_EVENT_SCREEN_UNLOADED, &ui_Screen_Setting);
    lv_obj_clear_flag(ui_Screen_Setting, LV_OBJ_FLAG_SCROLLABLE);  /// Flags
    lv_obj_set_style_bg_img_src(ui_Screen_Setting, &ui_img_bg1_png,
                                LV_PART_MAIN | LV_STATE_DEFAULT);
    Style_Init();
