...
static const foc_knob_param_t press_foc_knob_param = {
    0, 0, 10 * PI / 180, 2, 1, 1.1, ""};  // 按下旋转的力度
void set_screen2_index() { nvs_set_u8_data(NVS_SYS_HID_INDEX, icon_index); }
static void ui_icon_info_init() {
    uint8_t id;
    icon_index = sys_config.hid_sys_index;
    ui_icon_hid =
        heap_caps_malloc(ICON_CNT * sizeof(UI_HID_INFO), MALLOC_CAP_SPIRAM);
    ...
    ui_icon_hid[id].dial_sta[DIAL_STA_CLICK].hid_data[0] =
        HID_USAGE_CONSUMER_MUTE & 0xff;
    ui_icon_hid[id].dial_sta[DIAL_STA_CLICK].hid_data[1] =
        HID_USAGE_CONSUMER_MUTE >> 8;
    ui_icon_hid[id].dial_sta[DIAL_STA_R].hid_data[0] =
        HID_USAGE_CONSUMER_VOLUME_INCREMENT & 0xff;
    ui_icon_hid[id].dial_sta[DIAL_STA_R].hid_data[1] =
        HID_USAGE_CONSUMER_VOLUME_INCREMENT >> 8;
    ui_icon_hid[id].dial_sta[DIAL_STA_L].hid_data[0] =
        HID_USAGE_CONSUMER_VOLUME_DECREMENT & 0xff;
    ui_icon_hid[id].dial_sta[DIAL_STA_L].hid_data[1] =
        HID_USAGE_CONSUMER_VOLUME_DECREMENT >> 8;
...
void ui_Screen2_hid_event(uint8_t state) {
    if (screen_init_flag == 0) return;
    if (screen_init_flag == 1) {
        vTaskDelay(500 / portTICK_PERIOD_MS);
        screen_init_flag = 2;
    }
    static uint8_t hid_dial_press_flag =
        0;  // 用于是否在dial界面按下按键，判断按下旋转时切换icon发送按下旋转事件
    switch (state) {
...
static void Create(lv_obj_t* root) {
    ...
    lv_obj_add_style(ui_label.left, &style.font16,
                     LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_add_style(ui_label.right, &style.font16,
                     LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_arc_color(arc_bat, lv_color_hex(0x303030),
                               LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_arc_rounded(arc_bat, true,
                                 LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_set_style_arc_color(arc_bat, lv_color_hex(0xB0E14A),
                               LV_PART_INDICATOR | LV_STATE_DEFAULT);
    lv_obj_set_style_arc_opa(arc_bat, 255,
                             LV_PART_INDICATOR | LV_STATE_DEFAULT);
    lv_obj_set_style_arc_width(arc_bat, 6,
                               LV_PART_INDICATOR | LV_STATE_DEFAULT);
    lv_obj_set_style_arc_rounded(arc_bat, true,
                                 LV_PART_INDICATOR | LV_STATE_DEFAULT);

    lv_obj_set_style_bg_color(arc_bat, lv_color_hex(0xFFFFFF),
    ...
    // lv_obj_set_style_img_recolor(ble_img, lv_color_hex(0x000000),
    // LV_PART_MAIN | LV_STATE_DEFAULT);
    // lv_obj_set_style_img_recolor_opa(ble_img, 255, LV_PART_MAIN |
    // LV_STATE_DEFAULT);
...
void ui_Screen2_screen_init(void) {
    ...
    ui_icon_info_init();
    lv_obj_add_event_cb(ui_Screen2, scr_Screen2_loaded_cb,
                        LV_EVENT_SCREEN_LOADED, &ui_Screen2);
    // lv_obj_add_event_cb(ui_Screen2, scr_Screen2_unloaded_cb,
    // LV_EVENT_SCREEN_UNLOADED, &ui_Screen2);
    lv_obj_clear_flag(ui_Screen2, LV_OBJ_FLAG_SCROLLABLE);  /// Flags
    lv_obj_set_style_bg_img_src(ui_Screen2, &ui_img_bg1_png,
                                LV_PART_MAIN | LV_STATE_DEFAULT);
    Style_Init();
    Create(ui_Screen2);
    setl_label_info(icon_index);
