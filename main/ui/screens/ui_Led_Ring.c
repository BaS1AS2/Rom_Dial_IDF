// LVGL version: 8.4.0
// Project name: led ring Interface
// 在这个界面设置系统设置参数有：亮度、屏幕休眠、系统关机、屏幕旋转、wifi
#include "../ui.h"
#include "led_ring.h"

static const uint8_t ICON_CNT = 2;
static const char *TAG = "SCREEN_LED_RING";
...
typedef struct {
    ...
    struct UI_LED_RING_ICON_INFO icon;
} UI_LED_RINGICON_INFO;
static UI_LED_RINGICON_INFO *ui_icon;
static struct {
...
static foc_knob_param_t led_ring_foc_knob_param_lst[] = {
    [0] = {100, 0, 5 * PI / 180,  1, 1, 1.1, ""}, // 亮度0-100
    [1] = {100, 0, 5 * PI / 180,  1, 1, 1.1, ""}, // 速度1-300
...
static const foc_knob_param_t press_foc_knob_param = {
    0, 0, 20 * PI / 180, 1, 1, 1.1, ""};  // 按下旋转的力度
static void ui_icon_info_init() {
    uint8_t id;
    ui_icon = heap_caps_malloc(ICON_CNT * sizeof(UI_LED_RINGICON_INFO),
                               MALLOC_CAP_SPIRAM);
    memset(ui_icon, 0x00, ICON_CNT * sizeof(UI_LED_RINGICON_INFO));  // 全部清零

    id = 0;
    ui_icon[id].icon_id = id;
    ui_icon[id].param_list = led_ring_foc_knob_param_lst[id];
    ui_icon[id].icon.img_src = &ui_img_light_png;
    ui_icon[id].icon.name = "Brightness";
    ...
    ui_icon[id].icon_id = id;
    ui_icon[id].param_list = led_ring_foc_knob_param_lst[id];
    ui_icon[id].icon.img_src = &ui_img_speed_png;
    ui_icon[id].icon.name = "Speed";
    ui_icon[id].current_position = sys_config.set_light;
    ui_icon[id].param_list.position = ui_icon[id].current_position;
...
static void icon_light_event(uint8_t state) { 
    switch (state) {
        case DIAL_STA_R: 
            ...
            led_ring_set_brightness(
                (float)(ui_icon[icon_index].current_position) / 100.0);
            break;
...
static void icon_screenlock_event(uint8_t state) {
    switch (state) {
        case DIAL_STA_R:
            if (ui_icon[icon_index].current_position < 360)
                ui_icon[icon_index].current_position++;
            // set_screen_light(ui_icon[icon_index].current_position);
            led_ring_set_speed_deg(
                (float)(ui_icon[icon_index].current_position) * 20);
            break;
        case DIAL_STA_L:  // 屏幕亮度不能为0
            if (ui_icon[icon_index].current_position > 1)
                ui_icon[icon_index].current_position--;
...
static void icon_screen_direction_event(uint8_t state) {
    switch (state) {
        ...
        case DIAL_STA_L:
            ...
            break;
        case DIAL_STA_DOUBLE_CLICK:
            nvs_set_u8_data(SET_NVS_ROTATION,
                            ui_icon[icon_index].current_position);
            break;
        case DIAL_STA_P_R:
            nvs_set_u8_data(SET_NVS_ROTATION,
...
static void icon_first_screen_event(uint8_t state) {
    switch (state) {
        ...
        case DIAL_STA_L:
            ...
            break;
        case DIAL_STA_DOUBLE_CLICK:
            nvs_set_u8_data(SET_NVS_FIRST_SCREEN,
                            ui_icon[icon_index].current_position);
            break;
        case DIAL_STA_P_R:
            nvs_set_u8_data(SET_NVS_FIRST_SCREEN,
...
void ui_Screen_Led_Ring_event(uint8_t state) {
    switch (icon_index) {
        case 0:
    ...
    ui_icon[icon_index].param_list.position =
        ui_icon[icon_index].current_position;
    switch (state) {
...
static lv_obj_t *container_create(lv_obj_t *screen) {
    ...
    lv_obj_set_flex_align(ui_Container, LV_FLEX_ALIGN_CENTER,
    ...
    lv_obj_set_style_bg_color(ui_Container, lv_color_hex(0x000000),
                              LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_Container, 50, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_grad_color(ui_Container, lv_color_hex(0xFFFFFF),
                                   LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_main_stop(ui_Container, 0,
                                  LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_grad_stop(ui_Container, 255,
                                  LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_grad_dir(ui_Container, LV_GRAD_DIR_VER,
                                 LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(ui_Container, lv_color_hex(0xFF0000),
                                  LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_opa(ui_Container, 255,
                                LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(ui_Container, 1,
                                  LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_side(ui_Container, LV_BORDER_SIDE_TOP,
                                 LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_pad_left(ui_Container, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_pad_right(ui_Container, 0,
                               LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_pad_top(ui_Container, 10, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_pad_bottom(ui_Container, 0,
                                LV_PART_MAIN | LV_STATE_DEFAULT);
    return ui_Container;
}
/**
...
static void setl_label_info(uint8_t index) {
    ...
    switch (index) {
        case 0:  // 屏幕亮度
            ...
            lv_label_set_text_fmt(ui_label.mid, "%d",
                                  ui_icon[index].current_position);
            ui_icon[index].param_list.position =
                ui_icon[index].current_position - 1;
            break;
        case 1:  // 屏幕锁定时间
            lv_obj_add_flag(ui_label.wifi_roller, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(ui_label.bottom_label, LV_OBJ_FLAG_HIDDEN);
        ...
        case 2:  // 关机时间
            ...
            lv_obj_add_flag(ui_label.wifi_roller, LV_OBJ_FLAG_HIDDEN);
            lv_obj_add_flag(ui_label.bottom_label, LV_OBJ_FLAG_HIDDEN);
            lv_roller_set_options(ui_label.roller, SET_SYSTEM_SLEEP_TIME_SRC,
                                  LV_ROLLER_MODE_NORMAL);
            lv_roller_set_selected(ui_label.roller,
                                   ui_icon[index].current_position, LV_ANIM_ON);
            ui_icon[index].param_list.position =
                ui_icon[index].current_position;
            break;
        case 3:  // 屏幕方向
        ...
        case 4:  // WIFI开关
            ...
            lv_roller_set_options(ui_label.wifi_roller, "STA\nAP",
                                  LV_ROLLER_MODE_NORMAL);
            lv_roller_set_selected(ui_label.wifi_roller,
                                   ui_icon[index].current_position, LV_ANIM_ON);
            ui_icon[index].param_list.position =
                ui_icon[index].current_position;
            break;
...
static void Style_Init() {
    ...
    static const lv_style_prop_t style_prop[] = {LV_STYLE_TRANSLATE_X,
                                                 // LV_STYLE_TRANSLATE_Y,
                                                 // LV_STYLE_TRANSFORM_ZOOM,
                                                 LV_STYLE_PROP_INV};

    static lv_style_transition_dsc_t trans;
    lv_style_transition_dsc_init(&trans, style_prop, lv_anim_path_bounce, 200,
                                 0, NULL);
...
static void onFocus(lv_group_t *g) {
    ...
    if (current_btn_index > mid_btn_index) {
        icon_index++;
        if (icon_index > icon_num - 1) icon_index = 0;
        lv_obj_scroll_to_view(lv_obj_get_child(container, mid_btn_index - 1),
                              LV_ANIM_OFF);
    ...
    if (cnt > 1) {
        lv_img_set_zoom(lv_obj_get_child(container, mid_btn_index),
                        256);  /// ZOOM
        lv_obj_add_state(lv_obj_get_child(container, mid_btn_index - 1),
                         LV_STATE_USER_1);  /// States
        lv_img_set_zoom(lv_obj_get_child(container, mid_btn_index - 1),
                        220);  /// ZOOM
    }
    if (cnt > 2) {
        lv_obj_add_state(lv_obj_get_child(container, mid_btn_index + 1),
    ...
    if (cnt > 3) {
        lv_obj_add_state(lv_obj_get_child(container, mid_btn_index - 2),
                         LV_STATE_USER_2);  /// States
        lv_img_set_zoom(lv_obj_get_child(container, mid_btn_index - 2),
                        150);  /// ZOOM
    }
    if (cnt > 4) {
        lv_obj_add_state(lv_obj_get_child(container, mid_btn_index + 2),
                         LV_STATE_USER_3);  /// States
        lv_img_set_zoom(lv_obj_get_child(container, mid_btn_index + 2),
                        150);  /// ZOOM
    }
...
static void Create(lv_obj_t *root) {
    ...
    lv_obj_add_style(ui_label.name, &style.font20,
                     LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_add_style(ui_label.mid, &style.font16,
                     LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_arc_width(ui_label.arc, 8,
                               LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_set_style_arc_color(ui_label.arc, lv_color_hex(0x1A50BB),
                               LV_PART_INDICATOR | LV_STATE_DEFAULT);
    lv_obj_set_style_arc_opa(ui_label.arc, 100,
                             LV_PART_INDICATOR | LV_STATE_DEFAULT);
    lv_obj_set_style_arc_width(ui_label.arc, 8,
                               LV_PART_INDICATOR | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_bg_color(ui_label.roller, lv_color_hex(0x000000),
                              LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_label.roller, 50,
                            LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(ui_label.roller, lv_color_hex(0xFFFFFF),
                                  LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_opa(ui_label.roller, 100,
                                LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(ui_label.roller, 1,
                                  LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_set_style_text_color(ui_label.roller, lv_color_hex(0xFFFFFF),
                                LV_PART_SELECTED | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_label.roller, 255,
                              LV_PART_SELECTED | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(ui_label.roller, lv_color_hex(0x005BBB),
                              LV_PART_SELECTED | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_label.roller, 100,
                            LV_PART_SELECTED | LV_STATE_DEFAULT);
    ...
    lv_obj_set_style_bg_color(ui_label.wifi_roller, lv_color_hex(0x000000),
                              LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_label.wifi_roller, 50,
                            LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(ui_label.wifi_roller, lv_color_hex(0xFFFFFF),
                                  LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_opa(ui_label.wifi_roller, 100,
                                LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(ui_label.wifi_roller, 1,
                                  LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_set_style_text_color(ui_label.wifi_roller, lv_color_hex(0xFFFFFF),
                                LV_PART_SELECTED | LV_STATE_DEFAULT);
    lv_obj_set_style_text_opa(ui_label.wifi_roller, 255,
                              LV_PART_SELECTED | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(ui_label.wifi_roller, lv_color_hex(0x005BBB),
                              LV_PART_SELECTED | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(ui_label.wifi_roller, 100,
                            LV_PART_SELECTED | LV_STATE_DEFAULT);
    ...
    lv_obj_add_style(ui_label.bottom_label, &style.font16,
                     LV_PART_MAIN | LV_STATE_DEFAULT);
...
static void scr_Screen_Led_Ring_unloaded_cb(lv_event_t *e) {
    lv_obj_t **var = lv_event_get_user_data(e);
    lv_group_del(group);
    lv_obj_del(*var);
    (*var) = NULL;
}
static void scr_Screen_Led_Ring_loaded_cb(lv_event_t *e) {
    ui_state.index = UI_LED_RING_INTERFACE;
    ...
    switch (icon_index) {
        ...
        case 4:  // wifi
            ...
            if (wifi_state == 0) {
                ...
                lv_arc_set_value(ui_label.arc, wifi_press_time);
                lv_label_set_text_fmt(ui_label.bottom_label,
...
void ui_Screen_Led_Ring_screen_init(void) {
    ui_Led_Ring = lv_obj_create(NULL);
    ui_icon_info_init();
    lv_obj_add_event_cb(ui_Led_Ring, scr_Screen_Led_Ring_loaded_cb,
                        LV_EVENT_SCREEN_LOADED, &ui_Led_Ring);
    // lv_obj_add_event_cb(ui_Led_Ring, scr_Screen_Led_Ring_unloaded_cb,
    // LV_EVENT_SCREEN_UNLOADED, &ui_Led_Ring);
    lv_obj_clear_flag(ui_Led_Ring, LV_OBJ_FLAG_SCROLLABLE);  /// Flags
    lv_obj_set_style_bg_img_src(ui_Led_Ring, &ui_img_bg1_png,
                                LV_PART_MAIN | LV_STATE_DEFAULT);
    Style_Init();
    Create(ui_Led_Ring);
    setl_label_info(icon_index);
    lv_obj_t *container = container_create(ui_Led_Ring);
