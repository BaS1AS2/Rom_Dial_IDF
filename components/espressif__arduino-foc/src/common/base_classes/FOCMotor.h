...
enum TorqueControlType : uint8_t {
    ...
    dc_current =
        0x01,  //!< Torque control using DC current (one current magnitude)
...
enum FOCMotorStatus : uint8_t {
    ...
    motor_uncalibrated = 0x02,   //!< Motor is initialized, but not calibrated
                                 //!< (open loop possible)
    motor_calibrating = 0x03,    //!< Motor calibration in progress
    motor_ready =
        0x04,  //!< Motor is initialized and calibrated (closed loop possible)
    motor_error = 0x08,  //!< Motor is in error state (recoverable, e.g.
                         //!< overcurrent protection active)
    motor_calib_failed =
        0x0E,  //!< Motor calibration failed (possibly recoverable)
    motor_init_failed =
        0x0F,  //!< Motor initialization failed (not recoverable)
};
...
public:
     ...
     *
     * @param sensor Sensor class  wrapper for the FOC algorihtm to read the
     * motor angle and velocity
     */
     ...
     *
     * @param current_sense CurrentSense class wrapper for the FOC algorihtm to
     ...
     * electrical offset in respect to motor's electrical 0 position.
     ...
     * motor.controller If it is not set the motor will use the target set in
     * its variable motor.target
     *
    ...
    float
        voltage_bemf;  //!< estimated backemf voltage (if provided KV constant)
    ...
    FOCMotorStatus motor_status =
        FOCMotorStatus::motor_uninitialized;  //!< motor status
    ...
    PIDController PID_current_q{
        DEF_PID_CURR_P, DEF_PID_CURR_I, DEF_PID_CURR_D, DEF_PID_CURR_RAMP,
    ...
    LowPassFilter LPF_current_q{
        DEF_CURR_FILTER_Tf};  //!<  parameter determining the current Low pass
                              //!<  filter configuration
    LowPassFilter LPF_current_d{
        DEF_CURR_FILTER_Tf};  //!<  parameter determining the current Low pass
                              //!<  filter configuration
    PIDController PID_velocity{
        DEF_PID_VEL_P, DEF_PID_VEL_I, DEF_PID_VEL_D, DEF_PID_VEL_RAMP,
        DEF_PID_VEL_LIMIT};  //!< parameter determining the velocity PID
                             //!< configuration
    PIDController P_angle{
        DEF_P_ANGLE_P, 0, 0, 0,
        DEF_VEL_LIM};  //!< parameter determining the position PID configuration
    LowPassFilter LPF_velocity{
        DEF_VEL_FILTER_Tf};  //!<  parameter determining the velocity Low pass
                             //!<  filter configuration
    LowPassFilter LPF_angle{0.0};  //!<  parameter determining the angle low
                                   //!<  pass filter configuration
    unsigned int motion_downsample =
        DEF_MOTION_DOWNSMAPLE;  //!< parameter defining the ratio of
                                //!< downsampling for move commad
    unsigned int motion_cnt =
        0;  //!< counting variable for downsampling for move commad

    // sensor related variabels
    float sensor_offset;  //!< user defined sensor zero offset
    float zero_electric_angle =
        NOT_SET;  //!< absolute zero electric angle - if available
    int sensor_direction = NOT_SET;  //!< if sensor_direction == Direction::CCW
    ...
    /**
     * Utility function intended to be used with serial plotter to monitor motor
     * variables significantly slowing the execution down!!!!
     */
    void monitor();
    unsigned int monitor_downsample =
        DEF_MON_DOWNSMAPLE;  //!< show monitor outputs each monitor_downsample
                             //!< calls
    ...
    uint8_t monitor_variables =
        _MON_TARGET | _MON_VOLT_Q | _MON_VEL |
        _MON_ANGLE;  //!< Bit array holding the map of variables the user wants
                     //!< to monitor
